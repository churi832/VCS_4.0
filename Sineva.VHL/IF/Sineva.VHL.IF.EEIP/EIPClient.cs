...
{
    public enum EEIPConnectionType : byte
    {
        Null = 0,
        Multicast = 1,
        Point_to_Point = 2
    ...
    public enum EEIPRealTimeFormat : byte
    {
        Modeless = 0,
        ZeroLength = 1,
        Heartbeat = 2,
    ...
    //Read
    public enum EEIPAttribute : int
    {
        Group1_Entry_Count = 0x320,             //800
        Error_Info = 0x321,                     //801
        
        Output = 0x324,                         //804
        CurDistance = 0x325,                    //805

        Light_Intensity = 0x328,                //808
        
        Basic_Setting = 0x335,                  //821

        Initialization_Status = 0x339,          //825 //0:Idle, 1:Exe, 2:Finish 

        Group2_Entry_Count = 0x340,             //832
        Group1_SettingValue_Near = 0x341,       //833 //0 to 9999 (Default : 500)
        Group2_SettingValue_Near = 0x342,       //834 //0 to 9999 (Default : 500)

        Group1_SettingValue_Far = 0x344,        //836 //0 to 9999 (Default : 500)
        Group2_SettingValue_Far = 0x345,        //837 //0 to 9999 (Default : 500)

        Hysteresis_Value = 0x348,               //840

        DATUM_Offset = 0x34F,                   //847

        Group3_Entry_Count = 0x360,             //864
        Key_Lock_Status = 0x361,                //865
        Password = 0x362,                       //866

        NO_NC = 0x364,                          //868

        /// Response Time
        /// Laser Class1 : 0 -> 8ms, 1 -> 25ms, 2 -> 55ms, 3 -> 205ms, 4 -> 2000ms
        /// Laser Class2 : 0 -> 7ms, 1 -> 15ms, 2 -> 30ms, 3 -> 105ms, 4 -> 1000ms
        Response_Time = 0x365,                  //869 

        Hysteresis_Setting = 0x369,             //873

        Detection_Mode = 0x36B,                 //875
        Interference_Channel = 0x36C,           //876
        Received_Light_Sensitivity = 0x36D,     //877
        Stability_Filter = 0x36E,               //878
        Switching_Out = 0x36F,                  //879 // 0 : OutPut1, 1 :OutPut2

        Group4_Entry_Count = 0x380,             //896
        Output1_Timer = 0x381,                  //897 // 0 : Off, 1 : Off Delay, 2 : On Delay, 3 : One-Shot
        Output1_TimerValue = 0x382,             //898

        Output1_Mode = 0x384,                   //900

        External_Input_Func = 0x387,            //903

        Group5_Entry_Count = 0x3A0,             //928
        Output2_Timer = 0x3A1,                  //929
        Output2_TimerValue = 0x3A2,             //930

        Output2_Mode = 0x3A4,                   //932

        Output_Hold = 0x3AE,                    //942
        OnOff_Laser = 0x3AF,                    //943

        Group6_Entry_Count = 0x3C0,             //960
        Display_Screen = 0x3C6,                 //966
        Graphic_Mode = 0x3CF,                   //975
        Group7_Entry_Count = 0x3E0,             //992
        Brightness = 0x3E6,                     //998
        UserTag0 = 0x3EB,                       //1003
        UserTag1 = 0x3EC,                       //1004
        UserTag2 = 0x3ED,                       //1005
    }
    ...
    {
        #region Fields
        public static readonly EEIPClient Instance = new EEIPClient();
        ...
        private byte[] bufferArray = new byte[32];
        private int timeout = 1000;

        private string m_IpAddress = string.Format("192.168.1.221");
        #endregion
        ...
        /// </summary>
        public EEIPConnectionType O_T_ConnectionType { get; set; } = EEIPConnectionType.Point_to_Point;
        /// <summary>
        /// Connection Type Target -> Originator for Implicit Messaging (Default: ConnectionType.Multicast)
        /// </summary>
        public EEIPConnectionType T_O_ConnectionType { get; set; } = EEIPConnectionType.Multicast;
        /// <summary>
        ...
        /// </summary>
        public EEIPPriority O_T_Priority { get; set; } = EEIPPriority.Scheduled;
        /// <summary>
        /// Priority Target -> Originator for Implicit Messaging (Default: Priority.Scheduled)
        /// Could be: Priority.Scheduled; Priority.High; Priority.Low; Priority.Urgent
        /// </summary>
        public EEIPPriority T_O_Priority { get; set; } = EEIPPriority.Scheduled;
        /// <summary>
        ...
        /// </summary>
        public EEIPRealTimeFormat O_T_RealTimeFormat { get; set; } = EEIPRealTimeFormat.Header32Bit;
        /// <summary>
        /// Used Real-Time Format Target -> Originator for Implicit Messaging (Default: RealTimeFormat.Modeless)
        /// Possible Values: RealTimeFormat.Header32Bit; RealTimeFormat.Heartbeat; RealTimeFormat.ZeroLength; RealTimeFormat.Modeless
        /// </summary>
        public EEIPRealTimeFormat T_O_RealTimeFormat { get; set; } = EEIPRealTimeFormat.Modeless;
        /// <summary>
        ...
        public bool Initialize(string ipAddress, ushort portNo)
        {
            if (!m_Initialize)
            {
                if (nuObject == null)
                    nuObject = new NUObject(this);
                ...
                //{
                //    O_T_InstanceID = 0xfe;              //Instance ID of the Output Assembly
                //    O_T_Length = 0;
                //    O_T_RealTimeFormat = EEIPRealTimeFormat.Header32Bit;   //Header Format
                //    O_T_OwnerRedundant = false;
                //    O_T_Priority = EEIPPriority.Low;
                //    O_T_VariableLength = false;
                //    O_T_ConnectionType = EEIPConnectionType.Point_to_Point;
                //    RequestedPacketRate_O_T = 500000;    //RPI in  500ms is the Standard value

                //    //Parameters from Target -> Originator
                //    T_O_InstanceID = 0x64;
                //    T_O_Length = 128;
                //    T_O_RealTimeFormat = EEIPRealTimeFormat.Modeless;
                //    T_O_OwnerRedundant = false;
                //    T_O_Priority = EEIPPriority.Scheduled;
                //    T_O_VariableLength = false;
                //    T_O_ConnectionType = EEIPConnectionType.Point_to_Point;
                //    RequestedPacketRate_T_O = 500000;    //RPI in  500ms is the Standard value
        ...
        public UInt32 RegisterSession(UInt32 address, UInt16 port)
        {
            try
            {
                ...
                PingReply pingReply = ping.Send(ipAddress, timeout, bufferArray, pingOption);
        ...
        public void ForwardOpen(bool largeForwardOpen)
        {
            try
            {
                ...
                if (O_T_RealTimeFormat == EEIPRealTimeFormat.Header32Bit)
                    o_t_headerOffset = 6;
                if (O_T_RealTimeFormat == EEIPRealTimeFormat.Heartbeat)
                    o_t_headerOffset = 0;

                ushort t_o_headerOffset = 2;                    //Zählt den Sequencecount und evtl 32bit header zu der Länge dazu
                if (T_O_RealTimeFormat == EEIPRealTimeFormat.Header32Bit)
                    t_o_headerOffset = 6;
                if (T_O_RealTimeFormat == EEIPRealTimeFormat.Heartbeat)
                    t_o_headerOffset = 0;

                int lengthOffset = (5 + (O_T_ConnectionType == EEIPConnectionType.Null ? 0 : 2) + (T_O_ConnectionType == EEIPConnectionType.Null ? 0 : 2));
                ...
                commonPacketFormat.Data.Add((byte)((0x2) + (O_T_ConnectionType == EEIPConnectionType.Null ? 0 : 1) + (T_O_ConnectionType == EEIPConnectionType.Null ? 0 : 1)));
                //Verbindugspfad
                ...
                if (O_T_ConnectionType != EEIPConnectionType.Null)
                {
                    commonPacketFormat.Data.Add((byte)(0x2C));
                    commonPacketFormat.Data.Add((byte)(O_T_InstanceID));
                }
                if (T_O_ConnectionType != EEIPConnectionType.Null)
                {
                    commonPacketFormat.Data.Add((byte)(0x2C));
                ...
                if (O_T_ConnectionType == EEIPConnectionType.Multicast)
                {
                    UInt32 multicastResponseAddress = EEIPClient.GetMulticastAddress(BitConverter.ToUInt32(System.Net.IPAddress.Parse(m_IpAddress).GetAddressBytes(), 0));
                ...
                PingOptions pingOption = new PingOptions();
                PingReply pingReply = ping.Send(m_IpAddress, timeout, bufferArray, pingOption);
                if (pingReply.Status == IPStatus.Success)
                {
                    try
                    {
                ...
                while (itemCount > 2)
                {
                    ...
                    if (typeID == 0x8001)
                    {
                        ...
                        if (T_O_ConnectionType == EEIPConnectionType.Multicast)
        ...
        private void sendUDP()
        {
            ...
            while (!stopUDP)
            {
                ...
                System.Net.IPEndPoint endPointsend = new System.Net.IPEndPoint(System.Net.IPAddress.Parse(m_IpAddress), TargetUDPPort);
        ...
        public UInt32 RegisterSession()
        {
            ...
            return RegisterSession(m_IpAddress, this.TCPPort);
        }
        ...
        public int HeightSensorValue(byte[] analogInputs)
        {
            int value = 0;
            if(analogInputs.Length >= 2)
                value = (analogInputs[1] * 255 + analogInputs[0]);
            return value;
        }
    }
    #endregion
