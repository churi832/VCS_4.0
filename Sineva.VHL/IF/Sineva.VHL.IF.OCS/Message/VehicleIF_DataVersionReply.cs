...
namespace Sineva.VHL.IF.OCS
{
    public class VehicleIF_DataVersionReply : VehicleIF, IVehicleIF
    {
        ...
        public string JCSDataVersion = string.Empty;
        ...
        public bool SetDataFromStream(byte[] stream)
        {
            try
            {
                ...
                JCSDataVersion = Encoding.UTF8.GetString(stream, offset, 20).Trim('\0', ' '); offset += 20;
        ...
        public byte[] GetStreamFromDataOrNull()
        {
            try
            {
                ...
                if (JCSDataVersion.Length > 20)
                {
                    JCSDataVersion = JCSDataVersion.Substring(0, 20);
                }
                JCSDataVersion = JCSDataVersion.Trim().PadRight(20, '\0');
                byteStream.AddRange(Encoding.UTF8.GetBytes(JCSDataVersion));
        ...
        public List<String> GetLogData()
        {
            try
            {
                ...
                result.Add(string.Format("\t\tNode Data Version : {0}", NodeDataVersion.Trim('\0')));
                result.Add(string.Format("\t\tLink Data Version : {0}", LinkDataVersion.Trim('\0')));
                result.Add(string.Format("\t\tPort Data Version : {0}", PortDataVersion.Trim('\0')));
                result.Add(string.Format("\t\tJCS Data Version : {0}", JCSDataVersion.Trim('\0')));
                result.Add(string.Format("\t\tPIO Device Data Version : {0}", PIODeviceDataVersion.Trim('\0')));
