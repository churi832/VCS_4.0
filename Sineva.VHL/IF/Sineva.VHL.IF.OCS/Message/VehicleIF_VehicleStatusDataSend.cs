...
namespace Sineva.VHL.IF.OCS
{
    public class VehicleIF_VehicleStatusDataSend : VehicleIF, IVehicleIF
    {
        ...
        public int CurrentLinkNo = 0;
        public int CurrentVehiclePosition = 0;
        public int NextNodeNo = 0;
        public int TargetNodeNo = 0;
        public string CurrentCommandID = string.Empty;
        public bool UseFrontDetectionSensor = false; // 20231117 HJYOU OCS 추가 후 Message에 넣어야 함.
        public bool UseJCS = false;
        #endregion
        ...
        public bool SetDataFromStream(byte[] stream)
        {
            try
            {
                ...
                CurrentLinkNo = BitConverter.ToInt32(stream, offset); offset += 4;
                CurrentVehiclePosition = BitConverter.ToInt32(stream, offset); offset += 4;
                ...
                UseFrontDetectionSensor = Convert.ToBoolean(stream[offset++]);
                UseJCS = Convert.ToBoolean(stream[offset++]);
        ...
        public byte[] GetStreamFromDataOrNull()
        {
            try
            {
                ...
                byteStream.AddRange(BitConverter.GetBytes(CurrentLinkNo));
                byteStream.AddRange(BitConverter.GetBytes(CurrentVehiclePosition));
                ...
                byteStream.Add((UseFrontDetectionSensor == true) ? (byte)0x1 : (byte)0x0);
                byteStream.Add((UseJCS == true) ? (byte)0x1 : (byte)0x0);
        ...
        public List<String> GetLogData()
        {
            try
            {
                ...
                result.Add(string.Format("\t\tCurrent Link No.: {0}", CurrentLinkNo));
                result.Add(string.Format("\t\tCurrent Vehicle Position.: {0}", CurrentVehiclePosition));
                ...
                result.Add(string.Format("\t\tUse Fron Detection Sensor : {0}", UseFrontDetectionSensor));
                result.Add(string.Format("\t\tUse JCS : {0}", UseJCS));
