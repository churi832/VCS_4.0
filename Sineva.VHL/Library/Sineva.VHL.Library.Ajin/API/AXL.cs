...
namespace Sineva.VHL.Library.Ajin
{
    public class CAXL
    {
        ...
        // 라이브러리 버전 확인
        [DllImport("AXL.dll")]
        public static extern uint AxlGetLibVersion(ref byte szVersion);
        // Network제품의 각 모듈별 연결상태를 확인하는 함수
        [DllImport("AXL.dll")]
        public static extern uint AxlGetModuleNodeStatus(int nBoardNo, int nModulePos);
        // 해당 보드가 제어 가능한 상태인지 반환한다.
        [DllImport("AXL.dll")]
        public static extern uint AxlGetBoardStatus(int nBoardNo);
        // Network 제품의 Configuration Lock 상태를 반환한다.
        // *wpLockMode  : DISABLE(0), ENABLE(1)
        [DllImport("AXL.dll")]
        public static extern uint AxlGetLockMode(int nBoardNo, ref uint upLockMode);
        ...
        [DllImport("AXL.dll")]
        public static extern uint AxlScanStart(int lBoardNo, long lNet);
        // Network제품 각 보드의 모든 모듈을 connect하는 함수
        [DllImport("AXL.dll")]
        public static extern uint AxlBoardConnect(int lBoardNo, long lNet);
        // Network제품 각 보드의 모든 모듈을 Disconnect하는 함수
        [DllImport("AXL.dll")]
        public static extern uint AxlBoardDisconnect(int lBoardNo, long lNet);
    }
