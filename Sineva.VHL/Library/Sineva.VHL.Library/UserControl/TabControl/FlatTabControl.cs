...
namespace Sineva.VHL.Library
{
	...
	public class FlatTabControl : System.Windows.Forms.TabControl
	{
		...
		private Color mSelectedColor = SystemColors.Control;
		private Color mTextColor = SystemColors.Control;
		...
		internal void DrawTab(Graphics g, TabPage tabPage, int nIndex)
		{
			...
			if (bSelected)
			{
				Brush br1 = new SolidBrush(mySelectedColor);
			...
			//----------------------------
			// draw tab's icon
            if((tabPage.ImageIndex >= 0) && (ImageList != null) && (tabPage.ImageIndex < ImageList.Images.Count) && (ImageList.Images[tabPage.ImageIndex] != null))
			{
				int nLeftMargin = 8;
				int nRightMargin = 2;
			...
			else br = new SolidBrush(myTextColor);
		...
		[Browsable(true)]
		new public Color mySelectedColor
		{
			get { return mSelectedColor; }
			set { mSelectedColor = value; this.Invalidate(); }
		}
		[Browsable(true)]
		new public Color myTextColor
		{
			get { return mTextColor; }
			set { mTextColor = value; this.Invalidate(); }
		}
