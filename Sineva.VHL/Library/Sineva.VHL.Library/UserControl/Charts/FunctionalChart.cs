...
namespace Sineva.VHL.Library
{
	...
	public partial class FunctionalChart : Chart
	{
		public enum enZoomSpeed
		{
			Slow,
			Normal,
			Fast
		}
		private enZoomSpeed m_ZoomSpeed = enZoomSpeed.Normal;

		private double m_ZoomGain = 1.4;

		[Category("!ICS")]
		public enZoomSpeed ZoomSpeed
		{
			get { return m_ZoomSpeed; }
			set
			{
				if (value == enZoomSpeed.Fast)
					m_ZoomGain = 2;
				else if (value == enZoomSpeed.Normal)
					m_ZoomGain = 1.4;
				else if (value == enZoomSpeed.Slow)
					m_ZoomGain = 1.05;
			}
		}

		public FunctionalChart()
		{
			InitializeComponent();

			this.BorderSkin.SkinStyle = System.Windows.Forms.DataVisualization.Charting.BorderSkinStyle.Raised;
			...
			EnableZoomAndPanControls(this, ChartCursorSelected, ChartCursorMoved); ;
		}
		...
		private void Chart_MouseWheel(object sender, MouseEventArgs e)
		{
			try
			{
				double xMin = ChartAreas[0].AxisX.ScaleView.ViewMinimum;
				double xMax = ChartAreas[0].AxisX.ScaleView.ViewMaximum;
				double yMin = ChartAreas[0].AxisY.ScaleView.ViewMinimum;
				double yMax = ChartAreas[0].AxisY.ScaleView.ViewMaximum;

				double posXStart = 0.0;
				double posXFinish = 0.0;
				double posYStart = 0.0;
				double posYFinish = 0.0;

				if (e.Delta < 0)
				{
					posXStart = ChartAreas[0].AxisX.PixelPositionToValue(e.Location.X) - (xMax - xMin) * (double)m_ZoomGain;
					posXFinish = ChartAreas[0].AxisX.PixelPositionToValue(e.Location.X) + (xMax - xMin) * (double)m_ZoomGain;
					posYStart = ChartAreas[0].AxisY.PixelPositionToValue(e.Location.Y) - (yMax - yMin) * (double)m_ZoomGain;
					posYFinish = ChartAreas[0].AxisY.PixelPositionToValue(e.Location.Y) + (yMax - yMin) * (double)m_ZoomGain;
		...
		public static void ClearPoints(Series sender)
		{
		...
		/// </summary>
		public void EnableZoomAndPanControls(Chart sender)
		{
		...
		public void EnableZoomAndPanControls(Chart sender, CursorPositionChanged selectionChanged, CursorPositionChanged cursorMoved)
		{
		...
		/// </summary>
		/// <param name="sender"></param>
		public static void DisableZoomAndPanControls(Chart sender)
		{
			Chart ptrChart = sender;
		...
		/// <param name="sender"></param>
		/// <returns></returns>
		public static MSChartExtensionToolState GetChartToolState(Chart sender)
		{
			if (!ChartTool.ContainsKey(sender))
		...
		private static void FunctionalChart_MouseMove(object sender, MouseEventArgs e)
		{
			Point mousePoint = new Point(e.X, e.Y);
			Chart ptrChart = (Chart)sender;

			ptrChart.ChartAreas[0].CursorX.SetCursorPixelPosition(mousePoint, true);
			ptrChart.ChartAreas[0].CursorY.SetCursorPixelPosition(mousePoint, true);
		}
		...
		public static void DrawHorizontalLine(Chart sender, double y, Color lineColor, string name = "",
		...
		/// <param name="lineWidth">Line width</param>
		/// <param name="lineStyle">Line style</param>
		public static void DrawVerticalLine(Chart sender, double x, Color lineColor, string name = "", 
			int lineWidth = 1, ChartDashStyle lineStyle = ChartDashStyle.Solid)
		{
		...
		/// <param name="lineWidth">Line width</param>
		/// <param name="lineStyle">Line style</param>
		public static void DrawLine(Chart sender, double x0, double x1,
		...
		/// <param name="lineWidth">Line width</param>
		/// <param name="lineStyle">Line style</param>
		public static void DrawRectangle(Chart sender, double x, double y,
		...
		public static void AddText(Chart sender, string text,	double x, double y,
		...
		private void FunctionalChart_MouseDoubleClick(object sender, MouseEventArgs e)
		{
			DrawHorizontalLine(this, ChartAreas[0].AxisY.PixelPositionToValue(e.Location.Y), Color.Green);
			DrawVerticalLine(this, ChartAreas[0].AxisX.PixelPositionToValue(e.Location.X), Color.Green);
		}

		private void FunctionalChart_Click(object sender, EventArgs e)
		{

		}

		private void FunctionalChart_MouseDown(object sender, MouseEventArgs e)
		{

		}

		private void FunctionalChart_MouseUp(object sender, MouseEventArgs e)
		{
