...
namespace Sineva.VHL.Library.EasySocket
{
    ...
    public sealed class EasyServer : IDisposable
    {
        ...
        private float _ScanSleepTime = 100; // 10msec
        /// <summary>
       ...
       Description("Gets or sets list of clients IP address that the " +
        ...
        /// </exception>
        public EasyServer(bool UseSmartManagement)
        {
        ...
        /// </exception>
        public EasyServer(string ServerName, bool UseSmartManagement)
        {
            SmartManagementOn = UseSmartManagement;
            Name = ServerName;
            ServerPort = 2222;
        ...
        /// <c>TCPListener</c> using the localhost and the selected port.
        /// </exception>
        public EasyServer(int PortNumber, bool UseSmartManagement)
        {
        ...
        /// using the localhost and the selected port.
        /// </exception>
        public EasyServer(int PortNumber, string ServerIP, string ServerName, bool UseSmartManagement)
        {
            if (PortNumber <= 0) throw
                                     new EasySocketException("Server's port number must be greater than 0");
        ...
        {
            //VisionLog.WriteLog("SERVER : SEND - 1");
            ...
            lock (this)//Don't allow more then async data sending at a time
            {
                ...
                //VisionLog.WriteLog("SERVER : SEND - 2");
            ...
            if (OnDataSent != null && DataToSend.GetType().Name != "EasySocketCommands")
                OnDataSent(DataToSend);
            //VisionLog.WriteLog("SERVER : SEND - 3");
        ...
        private void ScanForNewClients()
        {
            ...
            while (true)
            {
                ...
                if (BlackListIPs != null && BlackListIPs.Contains(GetClientIP(NewClient)))
                {
        ...
        private void ScanForData()
        {
            while (true)
            {
                ...
                Thread.Sleep((int)(1000 / ScanSleepTime));
                //VisionLog.WriteLog("SERVER : Sleep = 1");
                ...
                for (short i = 0; i < ClientsSocketsList.Count; ++i)
                {
                    ...
                    {
                        //VisionLog.WriteLog("SERVER : RECV - 0");
                        ...
                        //VisionLog.WriteLog("SERVER : RECV - 1");
                        ...
                        {
                            string stringBuf = Data.ToString();
                            // Message 중복 처리
